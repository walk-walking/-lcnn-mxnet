...
import os
interpolation = cv2.INTER_CUBIC #插值方式   4*4像素领域的双3次插值
borderMode = cv2.BORDER_REPLICATE #添加的边框元素是边界元素的镜面反射
...
def crop_face(img, bbox, crop_sz, bbox_ext, extra_pad=0):
    ...
    if pad > 0:
        pad = pad + 3
        replicate = cv2.copyMakeBorder(img, pad, pad, pad, pad, borderMode) # 为原图像增加边界框
    else:
        replicate = img
...
if __name__ == "__main__":
    db_dir = '/home/centos/pad/lcnn-pytorch/dataset/CASIA_frames'
    save_dir = '/home/centos/pad/lcnn-pytorch/dataset/CASIA_scale'
